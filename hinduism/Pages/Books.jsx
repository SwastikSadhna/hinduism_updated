...
export default function Books() {
  ...
  try {
    const response = await axios.get(API_CONFIG.baseUrl + "/books/keywords");
  ...
  useEffect(() => {
    ...
    if (activeCategory) {
      ...
      const url = `${API_CONFIG.baseUrl}/books/filter`
      axios.get(url, {
        params: query
      }).then((res) => {
        setBhaktiTypes(res.data.data)
      ...
      <div className="max-w-7xl mx-auto">
        {
         <>
          <div className="text-center mb-4 flex justify-between">
              ...
              <button
              className="bg-orange-500 text-white my-auto px-4 py-2 h-100 rounded hover:bg-orange-600"
              onClick={toggleModal}
            >
              Filter
            </button>
...
<div className="mb-4">
  {filters.length > 0? filters.map(category => (
    <div key={category.name} >
      <h3 className='my-2 block text-center bg-slate-200 py-1 font-bold capitalize'>{category.name}</h3>
  ...
  )): <div className='font-bold text-red-500'>Sorry, no filters available</div>}
</div>
